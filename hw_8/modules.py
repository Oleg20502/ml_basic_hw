...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(gradOutput.shape[0]):
            ...
            self.gradInput[i] = np.matmul(gradOutput[i], np.diag(self.output[i])-np.matmul(output.T, output))
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(gradOutput.shape[0]):
            tmp = np.tile(np.exp(output[i]) / np.sum(np.exp(output[i])), n_in).reshape(n_in, n_in)
            self.gradInput[i] = gradOutput[i] - np.matmul(gradOutput[i], tmp)
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        grad_si = np.sum(gradOutput * (input - batch_mean), axis=0) * (-0.5*(batch_variance + self.EPS)**(-3/2))
        grad_mu = np.sum(gradOutput, axis=0) * (-1/np.sqrt(batch_variance + self.EPS))
        self.gradInput = gradOutput / np.sqrt(batch_variance + self.EPS) +                          grad_si * 2 * (input - batch_mean)/batch_size + grad_mu/batch_size
        return self.gradInput
